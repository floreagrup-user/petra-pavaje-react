    ...
    <div className="pt-20 md:pt-24">
      ...
      <div className="bg-anthracite-50 border-b border-anthracite-100">
        <div className="container-premium py-4">
          <nav className="flex items-center gap-2 text-sm">
            ...
            <span className="text-anthracite-900 font-medium">{info?.title}</span>
          </nav>
        </div>
      </div>
      ...
      <section className="section-padding bg-white pb-12">
        <div className="container-premium">
          ...
          >
            <h1 className="heading-h1 text-anthracite-900 mb-4">{info?.title}</h1>
            <p className="text-body-lg text-anthracite-500 max-w-3xl">{info?.description}</p>
          </motion.div>
        </div>
      </section>
      ...
      <section ref={ref} className="pb-16 md:pb-24">
        <div className="container-premium">
          {/* Toolbar */}
          <div className="flex items-center justify-between mb-8 pb-4 border-b border-anthracite-100">
            <p className="text-sm text-anthracite-500">
              {categoryProducts.length} produse
            </p>
            <div className="flex items-center gap-2">
              <button className="p-2 rounded-lg hover:bg-anthracite-50 transition-colors">
                <Filter className="w-5 h-5 text-anthracite-600" />
              </button>
              <div className="flex items-center gap-1 bg-anthracite-50 rounded-lg p-1">
                <button
                  onClick={() => setViewMode('grid')}
                  className={`p-1.5 rounded ${viewMode === 'grid' ? 'bg-white shadow-sm' : ''}`}
                >
                  <Grid className="w-4 h-4 text-anthracite-600" />
                </button>
                <button
                  onClick={() => setViewMode('list')}
                  className={`p-1.5 rounded ${viewMode === 'list' ? 'bg-white shadow-sm' : ''}`}
                >
                  <List className="w-4 h-4 text-anthracite-600" />
                </button>
              </div>
            </div>
          </div>
            ...
            {categoryProducts.map((product, index) => (
              ...
              >
                ...
                >
                  ...
                  <p className="text-sm text-anthracite-500 mb-2">{product.shortDescription}</p>
